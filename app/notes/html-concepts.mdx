...
You can only have one `<h1>` tag per page. For an article, this is usually the article title.

Do not skip heading levels. `<h2>` tags are for splitting the topic declared with `<h1>` into more specific subcategories. `<h3>` tags are for further divisions, and so on.
...
The `<ins>` and `<del>` tags are used to show content that has been either inserted or removed.
