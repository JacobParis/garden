...
So if I have a regular input, and I want to swap that out for a CurrencyInput component that has dollar sign formatting, or maybe we want to move it somewhere else in the form, as long as it's accessible and the label is roughly accurate, then getByRole('textbox', { name: /name/i }) is going to find the input, regardless of whether it's called name or username or display name or first name.
